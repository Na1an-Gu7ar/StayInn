...
public class VillaServiceImpl implements VillaService {
    ...
    public VillaResponseDTO createVilla(VillaCreateDTO villaCreateDTO) {
        ...
        villa.setImageUrls(villaCreateDTO.getImageUrls() != null ? 
                          new ArrayList<>(villaCreateDTO.getImageUrls()) : new ArrayList<>());
...
//    
    ...
    public List<VillaResponseDTO> searchVillas(String keyword) {
        ...
        return villaRepository.searchVillas(keyword).stream()
                .map(this::mapToResponseDTO)
                .collect(Collectors.toList());
    }
    ...
    public List<VillaResponseDTO> filterVillas(VillaSearchDTO searchDTO) {
        ...
        List<Villa> villas = villaRepository.findVillasWithFilters(
                searchDTO.getName(),
        ...
        List<VillaResponseDTO> results = villas.stream()
                .map(this::mapToResponseDTO)
                .collect(Collectors.toList());
    ...
    public List<VillaResponseDTO> getVillasByPriceRange(Double minPrice, Double maxPrice) {
        ...
        return villaRepository.findByPricePerNightBetween(minPrice, maxPrice).stream()
                .map(this::mapToResponseDTO)
                .collect(Collectors.toList());
    }
    ...
    public List<VillaResponseDTO> getVillasByAddress(String address) {
        ...
        return villaRepository.findByAddressContainingIgnoreCase(address).stream()
                .map(this::mapToResponseDTO)
                .collect(Collectors.toList());
    }
    ...
    public List<VillaResponseDTO> getTopRatedVillas(int limit) {
        ...
        return villaRepository.findTopRatedVillas().stream()
                .limit(limit)
                .map(this::mapToResponseDTO)
                .collect(Collectors.toList());
    }
    ...
    public List<VillaResponseDTO> getVillasSortedByPrice(boolean ascending) {
        ...
        List<Villa> villas = ascending ? 
                villaRepository.findAllByOrderByPricePerNightAsc() :
...
//    }
    ...
    private VillaDetailDTO mapToDetailDTO(Villa villa) {
        ...
        dto.setConfirmedBookings(bookingRepository.findByVillaIdAndStatus(
                villa.getId(), 
                com.stayinn.entities.BookingStatus.CONFIRMED
        ).size() + 0L);
